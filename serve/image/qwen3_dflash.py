...
class DFlashQwen3ForCausalLM(Qwen3ForCausalLM):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        ...
        # KV-cache names start after the target stack (layers.70+). This
        # checkpoint stores the draft as layers.0..N.
        layer_shift = 0
        for pname, _ in self.named_parameters():
            if ".layers." in pname:
                layer_shift = int(pname.split(".layers.", 1)[1].split(".", 1)[0])
                break
        shifted = []
        for name, loaded_weight in weights:
            if name.startswith("layers.") and layer_shift:
                idx_s, dot, rest = name[len("layers.") :].partition(".")
                if dot and idx_s.isdigit() and int(idx_s) < layer_shift:
                    name = f"layers.{int(idx_s) + layer_shift}.{rest}"
            shifted.append((name, loaded_weight))
        for name, loaded_weight in shifted:
