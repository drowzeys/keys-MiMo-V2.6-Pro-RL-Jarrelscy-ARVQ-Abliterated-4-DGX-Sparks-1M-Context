...
class MiMoV2Model(nn.Module, EagleModelMixin):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        print("ENTER MiMoV2Model.load_weights", flush=True)
        ...
        for name, loaded_weight in weights:
            ...
            # Checkpoint stores ARVQ tensors on mlp.experts.*; the module
            # registers them on mlp.experts.routed_experts.*.
            if ".mlp.experts." in name and ".routed_experts." not in name:
            ...
            if (
                shown_miss < 4
                and ("experts.arvq_" in name or name.endswith("hyb_kind"))
                and name not in params_dict
            ):
                tail = name.split("experts.", 1)[-1]
                close = [key for key in params_dict if key.endswith(tail)][:2]
                print(f"MISS {name} close={close}", flush=True)
                shown_miss += 1
